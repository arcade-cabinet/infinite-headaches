...
// ============================================================
// UI ZONES (Bank, Floor, Spawn indicators)
// ============================================================
...
interface ZoneProps {
  ...
  bankZoneY?: number;
  floorY?: number;
  showSpawnZone?: boolean;
  screenWidth: number;
  screenHeight: number;
}

/**
 * Renders game zones as 3D planes (mostly invisible, for visual reference)
 */
export const GameZones = ({
  ...
  useEffect(() => {
    ...
    // Floor line indicator (subtle)
    const floorLine = MeshBuilder.CreatePlane(
      "floorLine",
      { width: 20, height: 0.05 },
      scene
    );
    floorLine.position = new Vector3(0, floorY, 0);
    floorLine.rotation.x = Math.PI / 2;

    const floorMat = new StandardMaterial("floorLineMat", scene);
    floorMat.diffuseColor = new Color3(0.5, 0.4, 0.3);
    floorMat.alpha = 0.3;
    floorLine.material = floorMat;

    zonesRef.current.push(floorLine);

    // Bank zone indicator
    const bankZone = MeshBuilder.CreatePlane(
      "bankZone",
      { width: 2, height: 4 },
      scene
    );
    bankZone.position = new Vector3(bankZoneX, 0, -0.5);

    const bankMat = new StandardMaterial("bankMat", scene);
    bankMat.diffuseColor = new Color3(0.2, 0.8, 0.4);
    bankMat.alpha = 0.2;
    bankMat.emissiveColor = new Color3(0.1, 0.4, 0.2);
    bankZone.material = bankMat;

    zonesRef.current.push(bankZone);

    return () => {
      zonesRef.current.forEach((mesh) => mesh.dispose());
      zonesRef.current = [];
    };
  }, [scene, bankZoneX, floorY, showSpawnZone]);
